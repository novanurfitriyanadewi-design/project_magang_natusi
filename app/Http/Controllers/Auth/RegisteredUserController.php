...
class RegisteredUserController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        */
        ...
        $messages = [
            ...
            'anggota.*.cv_magang.required' =>
                'CV setiap anggota kelompok wajib diunggah.',

            'anggota.*.surat_pengajuan.required' =>
                'Surat pengantar setiap anggota kelompok wajib diunggah.',

            'anggota.*.cv_magang.mimes' =>
                'CV anggota harus berupa PDF, DOC, atau DOCX.',

            'anggota.*.surat_pengajuan.mimes' =>
                'Surat pengantar anggota harus berupa PDF, DOC, DOCX, JPG, JPEG, atau PNG.',
        ];
        ...
        */
        ...
        if ($roleSession === 'pelamar') {
            ...
            $validated['tipe_pengajuan'] =
                $validated['tipe_pengajuan'] ?? 'individu';

            $validated['jumlah_anggota'] =
                $validated['tipe_pengajuan'] === 'individu'
                    ? 1
                    : (int) (
                        $validated['jumlah_anggota'] ?? 1
                    );
            ...
            */
            ...
            if (
                $validated['tipe_pengajuan'] === 'kelompok'
                &&
                $validated['jumlah_anggota'] < 2
            ) {
                throw ValidationException::withMessages([
                    'jumlah_anggota' =>
                        'Pengajuan kelompok minimal terdiri dari 2 orang.',
                ]);
            }


            $anggotaTambahan = array_values(
                $validated['anggota'] ?? []
            );

            $expectedAdditional = max(
                0,
                $validated['jumlah_anggota'] - 1
            );
            ...
            if (
                count($anggotaTambahan)
                !==
                $expectedAdditional
            ) {
                throw ValidationException::withMessages([
                    'jumlah_anggota' =>
                        "Jumlah data anggota tambahan harus {$expectedAdditional} orang.",
                ]);
            }
            ...
            */
            ...
            if (
                ...
                throw ValidationException::withMessages([
                    'anggota' =>
                        'Email setiap anggota kelompok harus berbeda.',
                ]);
            }


            if (
                $allStudentIds->unique()->count()
                !==
                $allStudentIds->count()
            ) {
                throw ValidationException::withMessages([
                    'anggota' =>
                        'NIS/NIM setiap anggota kelompok harus berbeda.',
                ]);
            }
            ...
            */
            ...
            if (
                $additionalEmails->isNotEmpty()
                &&
                User::query()
                    ->whereIn(
                        'email',
                        $additionalEmails->all()
                    )
                    ->exists()
            ) {
                throw ValidationException::withMessages([
                    'anggota' =>
                        'Salah satu email anggota sudah digunakan oleh akun lain.',
                ]);
            }
            ...
            */
            ...
            if (
                ...
                throw ValidationException::withMessages([
                    'anggota' =>
                        'Salah satu email anggota sudah memiliki pengajuan magang aktif.',
                ]);
            }


            $additionalStudentIds = collect(
                $anggotaTambahan
            )
                ->pluck('no_induk')
                ->map(
            ...
            if (
                $additionalStudentIds->isNotEmpty()
                &&
                PermintaanMagang::query()
                    ->whereIn(
                        'no_induk',
                        $additionalStudentIds->all()
                    )
                    ->whereIn(
                        'status',
                        [
                ...
                throw ValidationException::withMessages([
                    'anggota' =>
                        'Salah satu NIS/NIM anggota sudah memiliki pengajuan magang aktif.',
                ]);
            }
            ...
            /*
            |--------------------------------------------------------------------------
            | CEK TABEL ANGGOTA MAGANG
            |--------------------------------------------------------------------------
            */

            $activeMemberEmailExists =
                PermintaanMagangAnggota::query()
                    ->whereIn(
                        'email',
                        $allEmails->all()
                    )
        ...
        */
        ...
        if ($roleSession === 'pelamar') {
            ...
            foreach (
                ...
                $cv = $request->file(
                    "anggota.{$index}.cv_magang"
                );

                $surat = $request->file(
                    "anggota.{$index}.surat_pengajuan"
                );
                ...
                if (! $cv || ! $surat) {
                    throw ValidationException::withMessages([
                        "anggota.{$index}.cv_magang" =>
                            'CV dan surat pengantar setiap anggota wajib diunggah.',
                    ]);
                }
        ...
        */
        ...
        if ($roleSession === 'pelamar') {
            ...
            $username =
                $this->makeApplicantUsernameFromEmail(
                    $email
                );
            ...
            DB::transaction(function () use (
                ...
                */
                ...
                $user = User::query()->create([
                    ...
                    /*
                    | Masih pelamar sampai diterima HRD
                    */
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | USERNAME UNTUK PEMOHON
    |--------------------------------------------------------------------------
    */

    private function makeApplicantUsernameFromEmail(
        string $email
    ): string {
    ...
    */
    ...
    private function makeUniqueUsername(
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | JIKA STUDENT ID KOSONG, PAKAI EMAIL
        |--------------------------------------------------------------------------
        */
