...
@section('content')
    ...
    {{-- Cara Pembayaran: QR Code (menggunakan data bank) --}}
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="flex flex-col items-center p-8">
            @php
                $bankWithQris = $banks->firstWhere('qris_image', '!=', null);
            @endphp
            @if ($bankWithQris)
                <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                    <img src="{{ asset('storage/'.$bankWithQris->qris_image) }}" alt="QR Pembayaran" class="h-[220px] w-[220px] object-contain">
                </div>
                ...
                @if ($bankWithQris->nama_bank)
                    <p class="mt-2 max-w-sm text-center text-xs text-slate-500">
                        Rekening: <span class="font-bold text-slate-700">{{ $bankWithQris->nama_bank }}</span>
                    </p>
                @endif
                @if ($nominalAktif)
                    <p class="mt-3 text-sm text-slate-500">Nominal: <span class="font-bold text-slate-900">Rp {{ number_format($nominalAktif->jumlah_nominal, 0, ',', '.') }}</span></p>
                @endif
            @else
                <svg class="h-16 w-16 text-slate-300 mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="8" height="8"/><rect x="14" y="2" width="8" height="8"/><rect x="2" y="14" width="8" height="8"/><rect x="14" y="14" width="8" height="8"/></svg>
    ...
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <form method="POST" action="{{ route('peserta-magang.pembayaran.store') }}" enctype="multipart/form-data" class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 uppercase mb-1">Pilih Rekening Tujuan</label>
