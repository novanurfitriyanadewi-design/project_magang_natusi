...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        {{-- Absensi Status --}}
        @php
            $labelStatusHariIni = match ($absensi['status_hari_ini']) {
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 p-5 text-white shadow-lg shadow-blue-200/40 transition hover:-translate-y-1 hover:shadow-xl">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm font-semibold text-blue-100">{{ $penugasan['selesai'] }} tugas sudah selesai</p>
    ...
    {{-- Breakdown Kehadiran & Tugas Belum Dikerjakan --}}
    <div class="grid grid-cols-1 gap-6 mb-6 lg:grid-cols-3">

        {{-- Breakdown Kehadiran Bulan Ini --}}
        <div class="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 class="text-lg font-semibold text-slate-900">Rincian Kehadiran {{ ($rentang ?? 'bulan') === 'minggu' ? 'Minggu Ini' : 'Bulan Ini' }}</h3>
            <p class="mb-4 text-sm text-slate-500">Jumlah hari berdasarkan status presensi Anda.</p>

            <div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
                <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-center">
                    <p class="text-2xl font-bold text-emerald-700">{{ $absensi['breakdown']['hadir'] }}</p>
                    <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-emerald-600">Hadir</p>
                </div>
                <div class="rounded-xl border border-amber-100 bg-amber-50 p-4 text-center">
                    <p class="text-2xl font-bold text-amber-700">{{ $absensi['breakdown']['terlambat'] }}</p>
                    <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-amber-600">Terlambat</p>
                </div>
                <div class="rounded-xl border border-blue-100 bg-blue-50 p-4 text-center">
                    <p class="text-2xl font-bold text-blue-700">{{ $absensi['breakdown']['izin'] }}</p>
                    <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-blue-600">Izin</p>
                </div>
                <div class="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-center">
                    <p class="text-2xl font-bold text-indigo-700">{{ $absensi['breakdown']['sakit'] }}</p>
                    <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-indigo-600">Sakit</p>
                </div>
                <div class="rounded-xl border border-rose-100 bg-rose-50 p-4 text-center">
                    <p class="text-2xl font-bold text-rose-700">{{ $absensi['breakdown']['alpa'] }}</p>
                    <p class="mt-1 text-xs font-semibold uppercase tracking-wide text-rose-600">Alpa</p>
                </div>
            </div>
        </div>

        {{-- Tugas Belum Dikerjakan Minggu Ini --}}
        <div class="flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 p-5">
                <h3 class="flex items-center gap-2 text-lg font-semibold text-slate-900">
                    <span class="material-symbols-outlined text-rose-600">pending_actions</span>
                    Belum Dikerjakan Minggu Ini
                </h3>
            </div>
            <div class="max-h-[280px] flex-1 overflow-y-auto">
                @forelse ($tugasBelumMingguIni as $item)
                    <div class="flex items-start justify-between gap-3 border-b border-slate-100 p-4">
                        <div class="min-w-0">
                            ...
                            <p class="mt-0.5 text-xs {{ $item['terlambat'] ? 'font-semibold text-rose-600' : 'text-slate-500' }}">
                                @if ($item['deadline'])
                                    Deadline {{ $item['deadline']->translatedFormat('D, d M') }}, {{ $item['deadline']->format('H:i') }}
                                    {{ $item['terlambat'] ? '(Terlewat)' : '' }}
                                @else
                                    Tanpa deadline
                                @endif
                            </p>
                        </div>
                        @if (Route::has('peserta-magang.penugasan.index'))
                            <a href="{{ route('peserta-magang.penugasan.index') }}" class="shrink-0 text-xs font-semibold text-blue-600 hover:underline">Kerjakan</a>
                        @endif
                    </div>
                @empty
                    <div class="flex h-full flex-col items-center justify-center p-8 text-center">
                        <span class="material-symbols-outlined mb-2 text-4xl text-emerald-400">task_alt</span>
                        <p class="text-sm text-slate-500">Semua tugas minggu ini sudah dikerjakan 🎉</p>
                    </div>
                @endforelse
            </div>
        </div>
