            ...
            @foreach($pengumuman as $item)
                @php
                    ...
                    $isIndividu = $item->penerima
                        ? $item->penerima->where('tipe_penerima', 'karyawan')->count() > 0
                        : false;
                @endphp

                {{-- CARD PENGUMUMAN --}}
                <article class="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md md:p-6">
    ...
    @else
        ...
        <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center shadow-sm">
            ...
            @else
                ...
                <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-400">
                    Belum ada informasi atau pengumuman terbaru dari perusahaan.
                </p>
            @endif
        </div>
