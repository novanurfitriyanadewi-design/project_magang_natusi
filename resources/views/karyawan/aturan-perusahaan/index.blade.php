...
@section('content')
    ...
    <div class="flex flex-col gap-4" id="rulesContainer">
        @forelse ($aturanList as $aturan)
            <div class="bg-white rounded-2xl border border-outline-variant overflow-hidden rule-item">
                <button type="button" class="w-full flex items-center justify-between p-5 text-left accordion-trigger">
                    <div class="flex-1">
                        <h2 class="font-semibold text-on-surface">{{ $aturan->nama }}</h2>
                        <p class="text-on-surface-variant text-sm line-clamp-1 accordion-preview">
                            {{ \Illuminate\Support\Str::limit($aturan->deskripsi, 100) }}
                        </p>
                    </div>
                    <span class="material-symbols-outlined chevron transition-transform duration-300">expand_more</span>
                </button>
