...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="flex flex-col gap-2">
        ...
        <p class="text-sm text-slate-500">Presensi "Hadir" dan "Pulang" hanya dapat dilakukan dari area kantor CV Natusi.</p>
    </div>
            ...
            <section class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                ...
                @elseif ($absensiHariIni->status === 'hadir' && !$absensiHariIni->jam_keluar)
                    ...
                    <h2 class="text-lg font-semibold text-slate-900 mb-2 flex items-center gap-2">
                        <span class="material-symbols-outlined text-blue-600">logout</span>
                        Presensi Pulang
                    </h2>
                    <p class="text-sm text-slate-500 mb-6">
                        Anda checkin pukul {{ \Carbon\Carbon::parse($absensiHariIni->jam_masuk)->format('H:i') }} WIB. Jangan lupa presensi pulang sebelum meninggalkan kantor.
                    </p>

                    <form
                        method="POST"
                        action="{{ route('karyawan.absensi.pulang') }}"
                        class="space-y-6"
                        x-data="{
                            lat: null,
                            lng: null,
                    ...
                    >
                        @csrf
                        <div class="rounded-xl p-4 border" :class="inRange ? 'bg-green-50 border-green-200' : 'bg-rose-50 border-rose-200'">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-2">
                                    <span class="material-symbols-outlined text-xl" :class="inRange ? 'text-green-600' : 'text-rose-600'">location_on</span>
                                    <span class="font-semibold text-sm">Lokasi Saat Ini</span>
                        ...
                        <button
                            type="submit"
                            :disabled="!inRange"
                            :class="inRange ? 'bg-blue-600 hover:bg-blue-700 cursor-pointer' : 'bg-slate-300 cursor-not-allowed'"
                            class="w-full text-white font-bold py-3 px-6 rounded-xl transition-all shadow-lg shadow-blue-600/20"
                        >
                            <span x-show="inRange">Submit Presensi Pulang</span>
                            <span x-show="!inRange">Di Luar Area Kantor — Tidak Dapat Submit</span>
                        </button>
                    </form>

                @else
                    {{-- SUDAH LENGKAP (hadir + pulang) ATAU STATUS IZIN/SAKIT --}}
                    <h2 class="text-lg font-semibold text-slate-900 mb-6 flex items-center gap-2">
                        <span class="material-symbols-outlined text-blue-600">edit_calendar</span>
                        Input Kehadiran
                    </h2>
                    <div class="rounded-xl border border-green-200 bg-green-50 p-4 text-green-700 text-sm flex items-center gap-2">
                        ...
                        @if ($absensiHariIni->status === 'hadir')
                            Presensi hari ini lengkap — masuk {{ \Carbon\Carbon::parse($absensiHariIni->jam_masuk)->format('H:i') }} WIB, pulang {{ \Carbon\Carbon::parse($absensiHariIni->jam_keluar)->format('H:i') }} WIB. Sampai jumpa besok!
                        @else
                            Anda tercatat {{ $absensiHariIni->status }} untuk hari ini. Sampai jumpa besok!
                        @endif
                    </div>
                @endif
            </section>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-200">
                                ...
                                <th class="px-6 py-4 font-semibold text-sm text-slate-700">Masuk</th>
                                <th class="px-6 py-4 font-semibold text-sm text-slate-700">Pulang</th>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                ...
                <ul class="text-sm text-slate-500 space-y-3">
                    ...
                    <li class="flex gap-2">
                        <span class="text-blue-600 font-bold">•</span>
                        Absensi 'Hadir' dan 'Pulang' hanya dapat dilakukan dalam radius {{ $radiusMeters }} meter dari kantor CV Natusi.
                    </li>
                    <li class="flex gap-2">
                        <span class="text-blue-600 font-bold">•</span>
