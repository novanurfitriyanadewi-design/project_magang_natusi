...
<div x-data="{
    ...
    statusLabel(status) {
        ...
        if (status === 'perlu_revisi') return 'Perlu Revisi';
        return 'Menunggu';
    },
                        ...
                        @php
                            ...
                            $anggotaPayload = $item->anggota->map(fn ($anggota) => [
                                ...
                                'cv_url' => ($anggota->cv_path ?: ($anggota->is_ketua ? $item->cv_path : null))
                                    ? \Illuminate\Support\Facades\Storage::disk('public')->url($anggota->cv_path ?: $item->cv_path)
                                    : null,
                                'surat_url' => ($anggota->surat_pengajuan_path ?: ($anggota->is_ketua ? $item->surat_pengajuan_path : null))
                                    ? \Illuminate\Support\Facades\Storage::disk('public')->url($anggota->surat_pengajuan_path ?: $item->surat_pengajuan_path)
                                    : null,
                            ...
                            if (empty($anggotaPayload)) {
                                $anggotaPayload[] = [
                                    ...
                                    'no_hp' => $noHp,
                                    'is_ketua' => true,
                                    'cv_url' => $item->cv_path ? \Illuminate\Support\Facades\Storage::disk('public')->url($item->cv_path) : null,
                                    'surat_url' => $item->surat_pengajuan_path ? \Illuminate\Support\Facades\Storage::disk('public')->url($item->surat_pengajuan_path) : null,
                            ...
                            $revisiPayload = $item->riwayatBerkas->map(fn ($berkas) => [
                                ...
                                'url' => \Illuminate\Support\Facades\Storage::disk('public')->url($berkas->path),
