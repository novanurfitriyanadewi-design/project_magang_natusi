...
<div x-data="taskTemplatePage()" class="space-y-6">
    ...
    <section class="overflow-hidden rounded-3xl border border-sky-100/90 bg-white/95 shadow-[0_20px_50px_rgba(15,52,94,0.09)] backdrop-blur">
        ...
        <div class="grid gap-6 p-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)]">
            <form action="{{ route('admin-peserta.tugas.template-laporan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                    ...
                    @forelse ($templateLaporan as $template)
                        <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <form action="{{ route('admin-peserta.tugas.template-laporan.destroy', $template) }}" method="POST" onsubmit="return confirm('Hapus template laporan ini?');">
                                    @csrf @method('DELETE')
    ...
    <section id="daftar-penugasan" class="overflow-hidden rounded-3xl border border-sky-100/90 bg-white/95 shadow-[0_20px_50px_rgba(15,52,94,0.09)] backdrop-blur">
        ...
        <div class="border-b border-sky-100 bg-gradient-to-r from-sky-50 via-blue-50 to-cyan-50 px-6 py-5">
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-slate-500">Susunan tabel mengikuti template Excel: Minggu Ke, Materi &amp; Laporan, Tugas, Hari Tampil, Hari Deadline, dan Jam Deadline.</p>
                </div>
            </div>
        </div>
        ...
        @php
            ...
            if (!array_key_exists($kelompokAktif, $kelompokMeta)) $kelompokAktif = '';
            $kelompokDitampilkan = $kelompokAktif !== '' ? [$kelompokAktif] : array_keys($kelompokMeta);
        @endphp
        ...
        <div class="border-b border-slate-200 bg-slate-50/70 px-6 py-4">
            <div class="flex flex-wrap items-center gap-2" aria-label="Filter kelompok peserta">
                @foreach ($filterKelompok as $filter)
                    @php $aktif = $kelompokAktif === $filter['value']; $filterUrl = $filter['value'] === '' ? route('admin-peserta.tugas.index') : route('admin-peserta.tugas.index', ['target_peserta' => $filter['value']]); @endphp
