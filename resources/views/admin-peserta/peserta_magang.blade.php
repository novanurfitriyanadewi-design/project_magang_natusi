...
<div class="space-y-5" x-data="{
    ...
    detailOpen: false,
    detail: {},
    startImport() { this.importLoading = true; this.importOpen = false; },
    closeImportResult() { this.importResultOpen = false; },
    openDetail(payload) { this.detail = payload; this.detailOpen = true; },
    ...
    <section class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-1 max-w-3xl text-sm leading-6 text-slate-500">Peserta baru otomatis masuk setelah pengajuan disetujui. Import Excel hanya digunakan untuk memasukkan data peserta lama.</p>
        </div>
        <div class="flex flex-col gap-2 sm:flex-row">
            <a href="{{ route('admin-peserta.peserta.template') }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-sky-200 bg-white px-4 py-2.5 text-sm font-bold text-sky-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-sky-50">
            ...
            <button type="button" @click="importOpen = true" class="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-600 to-cyan-500 px-4 py-2.5 text-sm font-bold text-white shadow-[0_12px_28px_rgba(2,132,199,0.24)] transition hover:-translate-y-0.5 hover:from-sky-700 hover:to-cyan-600">
    ...
    <section class="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-[0_14px_40px_rgba(15,23,42,0.06)]">
                        ...
                        <tr class="transition hover:bg-sky-50/40">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('admin-peserta.peserta.status', $item->id_peserta) }}" method="POST" onsubmit="return confirm('{{ $isActive ? 'Nonaktifkan peserta ini?' : 'Aktifkan kembali peserta ini?' }}')">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="status" value="{{ $isActive ? 'nonaktif' : 'aktif' }}">
                                        <button class="inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-bold transition {{ $isActive ? 'border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100' : 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100' }}">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $isActive ? 'M5 15l-2-2m0 0l2-2m-2 2h14m-6 6l2-2m0 0l-2-2m2 2V5' : 'M15 9l2-2m0 0l-2-2m2 2h-4m6 6l-2 2m0 0l2 2m-2-2v4' }}"/></svg>
        ...
        @if($peserta->hasPages())
            ...
            <div class="flex flex-col gap-3 border-t border-sky-100 bg-sky-50/50 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-xs font-semibold text-slate-500">Menampilkan {{ $peserta->firstItem() ?? 0 }}–{{ $peserta->lastItem() ?? 0 }} dari {{ number_format($peserta->total()) }} peserta</p>
                <nav class="inline-flex w-fit overflow-hidden rounded-xl border border-sky-200 bg-white shadow-sm" aria-label="Navigasi halaman peserta">
                    ...
                    @foreach(range($startPage, $endPage) as $page)
                        @if($page === $peserta->currentPage())
                            <span class="inline-grid h-11 min-w-11 place-items-center border-r border-sky-200 bg-gradient-to-br from-sky-500 to-blue-600 px-3 text-sm font-extrabold text-white shadow-inner" aria-current="page">{{ $page }}</span>
                        @else
                            <a href="{{ $peserta->url($page) }}" class="inline-grid h-11 min-w-11 place-items-center border-r border-sky-200 bg-white px-3 text-sm font-bold text-sky-700 transition hover:bg-sky-50">{{ $page }}</a>
                        @endif
                    @endforeach
                    ...
                    @if($endPage < $peserta->lastPage())
                        @if($endPage < $peserta->lastPage() - 1)
                            <span class="inline-grid h-11 min-w-10 place-items-center border-r border-sky-200 bg-white px-2 text-sm font-bold text-sky-400">…</span>
                        @endif
                        <a href="{{ $peserta->url($peserta->lastPage()) }}" class="inline-grid h-11 min-w-11 place-items-center border-r border-sky-200 bg-white px-3 text-sm font-bold text-sky-700 transition hover:bg-sky-50">{{ $peserta->lastPage() }}</a>
                    @endif
        ...
        <div x-show="importOpen" x-cloak x-transition.opacity class="fixed inset-0 z-[9999] flex h-screen w-screen items-center justify-center overflow-y-auto bg-slate-950/70 p-4 backdrop-blur-md" @click.self="if (!importLoading) importOpen = false">
            <section x-show="importOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" class="my-auto w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-[0_30px_80px_rgba(15,23,42,0.30)]">
                <header class="flex items-start justify-between gap-4 border-b border-sky-100 bg-gradient-to-r from-sky-50 via-blue-50 to-cyan-50 px-6 py-5">
                    <div>
                        <span class="inline-flex items-center rounded-full bg-white px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-sky-700 ring-1 ring-sky-200">Data Lama</span>
                    ...
                    <button type="button" @click="importOpen = false" class="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:opacity-50">
                        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </header>

                <form action="{{ route('admin-peserta.peserta.import') }}" method="POST" enctype="multipart/form-data" class="space-y-5 px-6 py-6" @submit="startImport()">
                    @csrf
                    ...
                    <label class="block cursor-pointer rounded-2xl border-2 border-dashed border-sky-200 bg-slate-50 px-5 py-8 text-center transition hover:border-sky-400 hover:bg-sky-50">
                        ...
                        <input type="file" name="file_excel" accept=".xlsx,.xls,.csv" :disabled="importLoading" class="mt-4 block w-full text-xs text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-sky-100 file:px-3 file:py-2 file:font-bold file:text-sky-700 disabled:cursor-not-allowed disabled:opacity-60" required>
                    </label>

                    <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                        <button type="button" @click.stop="importOpen = false; importLoading = false" class="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-100">Batal</button>
                        <button type="submit" :disabled="importLoading" class="inline-flex min-w-[145px] items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-sky-600 to-cyan-500 px-5 py-2.5 text-sm font-bold text-white shadow-[0_10px_24px_rgba(2,132,199,0.22)] transition hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-80">
    ...
    <template x-teleport="body">
        <div x-show="importLoading" x-cloak x-transition.opacity class="fixed inset-0 z-[10000] flex h-screen w-screen items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md">
            <section x-show="importLoading" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-4 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" class="w-full max-w-sm rounded-3xl bg-white px-8 py-9 text-center shadow-[0_30px_90px_rgba(15,23,42,0.35)]" role="status" aria-live="polite">
                ...
                <p class="mt-2 text-sm leading-6 text-slate-500">Sistem sedang membaca file dan menyimpan data. Jangan menutup atau memuat ulang halaman.</p>
    ...
    <template x-teleport="body">
        <div x-show="importResultOpen" x-cloak x-transition:enter="transition ease-out duration-250" x-transition:enter-start="opacity-0 translate-y-3 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" x-transition:leave="transition ease-in duration-180" x-transition:leave-start="opacity-100 translate-y-0 scale-100" x-transition:leave-end="opacity-0 translate-y-3 scale-95" x-init="if (importResultOpen) setTimeout(() => importResultOpen = false, 7000)" class="fixed right-4 top-4 z-[10001] w-[calc(100%-2rem)] max-w-md" role="alert" aria-live="assertive">
            <div :class="importResultType === 'success' ? 'border-emerald-200 bg-white shadow-[0_20px_55px_rgba(5,150,105,0.20)]' : 'border-rose-200 bg-white shadow-[0_20px_55px_rgba(225,29,72,0.20)]'" class="flex items-start gap-4 rounded-2xl border p-4">
                <span :class="importResultType === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'" class="grid h-11 w-11 shrink-0 place-items-center rounded-xl">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" :d="importResultType === 'success' ? 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' : 'M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'"/>
                    </svg>
                </span>
                <div class="min-w-0 flex-1">
                    <p :class="importResultType === 'success' ? 'text-emerald-800' : 'text-rose-800'" class="text-sm font-extrabold" x-text="importResultType === 'success' ? 'Import berhasil' : 'Import gagal'"></p>
                    <p class="mt-1 break-words text-sm leading-6 text-slate-600" x-text="importResultMessage"></p>
                </div>
                <button type="button" @click="closeImportResult()" class="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" aria-label="Tutup notifikasi">
